...
Proiectul este dezvoltat în Unity 6000.4.2f1 utilizând Universal Render Pipeline (URP) și New Input System. Arhitectura software este organizată modular, cu componente separate pentru fizica vehiculului, inteligența artificială, generarea traseului și interfața utilizator.
...
\begin{itemize}
    ...
    \item \textbf{Editor/} -- scripturi de configurare automată (\texttt{AutoSetupScene}, \texttt{AutoSpawnRoad})
\end{itemize}
...
\subsection{Generarea Procedurală a Traseului}

Clasa \texttt{AutoSpawnRoad} generează automat un circuit octogonal. Algoritmul calculează:

\begin{enumerate}
    \item Raza octogonului intern folosind relația trigonometrică:
    \begin{equation}
        R = \frac{L / 2}{\tan(22.5°)}
    \end{equation}
    unde $L = 100$ este lungimea unei laturi.
    
    \item Razele exterioare și interioare:
    \begin{equation}
        R_{ext} = R + w/2 + 0.5, \quad R_{int} = R - w/2 - 0.5
    \end{equation}
    unde $w = 30$ este lățimea pistei.
    
    \item Lungimea pereților pentru a forma un octogon perfect:
    \begin{equation}
        L_{perete} = 2 \cdot R_{perete} \cdot \tan(22.5°) + 2
    \end{equation}
    
    \item Pentru fiecare din cele 8 segmente, se generează un segment de drum, un perete exterior și un perete interior, poziționate la unghiuri de $i \cdot 45°$.
\end{enumerate}
